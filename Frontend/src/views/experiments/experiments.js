...
var experiments = View.extend({
	...
	experimentsTableRender(){
		this.experimentsCollection.fetch({
			...
			/*	console.log(res.toJSON());
				...
				*/
				console.log($('#experimentsUnderProject'));
				    	...
				    	},
				    	{
				            ...
				            "render": _.bind(function ( data, type, full, meta ) {
				            	...
				            	if(full.number_of_images){
				            		return "<a class='fa fa-download expDownloadProgress' style='cursor:pointer' exp_id='"+full.id+"' exp_name='"+full.title.replace(/[%/]/g,'_')+"' title='"+full.id+'/'+full.title.replace(/[%/]/g,'_')+"'></a><a class='fa fa-spinner fa-spin' style='display:none;'></a><progress class=' progress' value='0' style='display:none;margin-right:5px;width:16px;font-size:5px;height:5px;margin-bottom:0px'></progress>  </a><a class='fa icon-trash' style='cursor: pointer;color:red;font-size:18px'></a>"
				            	}
				            	else{
				            		return "<a class='fa icon-trash' style='cursor: pointer;color:red;font-size:18px'></a>"
				            	}
				            },this)
				    	},
				    ],
          ...
          let spinnerIcon=$(e.target).next('.fa-spinner');
					...
					let exp_name = $(e.currentTarget).attr('exp_name');
					let exp_id = $(e.currentTarget).attr('exp_id');
	...
	participantsInfo(e){
		if(!$(e.target).hasClass('fa-download')&&!$(e.target).hasClass('icon-trash')){
	...
	changeUserPermission(e){
		...
		let user_id=e.currentTarget.parentElement.parentElement.id;
		let permissions;
		...
		//console.log(e);
		let newUserPermissions = new FormData();
	...
	create_experiment(){
		...
		console.log($('#experiment_name').val());
