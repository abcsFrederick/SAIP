...
var protocol = View.extend({
  ...
  render () {
    this.statisticsCollection.fetch({
      ...
      success: (_.bind(function (res) {
        ...
        const fromdate = new Date('2018-01-01')
        ...
        const fromdate_year = fromdate.getUTCFullYear()
        const fromdate_string = fromdate_year + '-' + fromdate_month + '-' + fromdate_day
        $('#slider-range').slider({
		      range: true,
  ...
  //   let max = max.getTime();
    ...
    const newdata = data.filter(function (x) {
    	const time = new Date(x.timestamp).getTime()
  ...
  pieChartRenderUser (data) {
    ...
    this.uniqueUser.forEach(_.bind(function (userId, index) {
      ...
      const heatmapPercent = record.num_of_actions / data.length * 100
      console.log(record.num_of_actions)
      const hue = (50 - heatmapPercent * 0.5) / 360
      const heatmapColor = this.hslToRgb(hue, 1, 0.5)
      this.randomColor.push(heatmapColor)
    }, this))

    // console.log($(".pieChart").width());
    const width = $('.pieChart').width()
		    const height = $('.pieChart').width()
    const radius = Math.min(width * 0.8, height) / 2

    const g = this.svg.attr('transform', 'translate(' + width / 2 + ',' + height / 2 + ')')
    ...
    const arc = g.selectAll('.arc')
      ...
      .enter().append('g')
      .classed('arc', true)
    const pathArea = arc.append('path')
      .attr('d', path)
  ...
  pieChartRenderEvent (data) {
    ...
    this.uniqueEvent.forEach(_.bind(function (event, index) {
      ...
      const hue = (190 + heatmapPercent * 0.5) / 360
      console.log(hue)
      const heatmapColor = this.hslToRgb(hue, 1, 0.5)
      this.randomColor_event.push(heatmapColor)
    }, this))

    const width = $('.pieChartEvent').width()
		    const height = $('.pieChartEvent').width()
    const radius = Math.min(width * 0.8, height) / 2

    const g = this.svg_event.attr('transform', 'translate(' + width / 2 + ',' + height / 2 + ')')
    ...
    // g = this.svg.attr("transform", "translate(" + width / 2 + "," + height / 2 + ")");
    const arc = g.selectAll('.arcEvent')
      .data(pie(pieChartDisplay))
      .enter().append('g')
