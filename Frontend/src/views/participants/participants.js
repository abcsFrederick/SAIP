      ...
      success: _.bind(function (res) {
        ...
        if (res.toJSON()[0].error != 'no patient exist') {
          ...
          this.participantsTable = $('#participantsTable').DataTable({
            data: patientsWithStudies,
                  ...
                  // console.log(full.series_description);
                    ...
                    full.series_description[i] = this.stringFilter(full.series_description[i])
                  }
                  return '<a class=\'fa fa-download studyDownloadProgress\' style=\'cursor:pointer\' stu_id=\'' + full.study_id + '\' stu_name=\'' + this.stringFilter(full.study_description) + '\' title=\'' + full.pat_path + '/' + full.study_path + '/' + this.stringFilter(full.pat_name) + '/' + this.stringFilter(full.study_description) + '?series_path=' + full.series_path + '&series_description=' + full.series_description + '&modality=' + full.modality + '\'></a><a class=\'fa fa-spinner fa-spin\' style=\'display:none;\'></a><progress class=\' progress\' value=\'0\' style=\'display:none;margin-right:5px;width:16px;font-size:5px;height:5px;margin-bottom:0px\'></progress>'
					            }, this)
					    	}
					    ],
					    destroy: true,
          ...
          $('.studyDownloadProgress').on('click', _.bind(function (e) {
            ...
            const stu_name = $(e.currentTarget).attr('stu_name')
            const stu_id = $(e.currentTarget).attr('stu_id')
  ...
  series (e) {
    if (!$(e.target).hasClass('fa-download')) {
        ...
        columns: [
          {
					    	...
					    	},
					    	{
					            ...
					            render: _.bind(function (data, type, full, meta) {
					            	...
					            	return '<a class=\'fa fa-download seriesDownloadProgress\' style=\'cursor:pointer\' ser_id=\'' + full.series_path + '\' ser_name=\'' + this.stringFilter(full.series_description) + '\' title=\'' + this.patient_path + '/' + this.study_path + '/' + full.series_path + '/' + this.stringFilter(full.series_description) + '/' + full.modality + '\'></a><a class=\'fa fa-spinner fa-spin\' style=\'display:none;\'></a><progress class=\' progress\' value=\'0\' style=\'display:none;margin-right:5px;width:16px;font-size:5px;height:5px;margin-bottom:0px\'></progress>'
					            }, this)
					    	}
					    ],
					    destroy: true,
      ...
      $('.seriesDownloadProgress').on('click', _.bind(function (e) {
        ...
        const ser_name = $(e.currentTarget).attr('ser_name')
        const ser_id = $(e.currentTarget).attr('ser_id')
