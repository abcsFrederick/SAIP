...
var users = Backbone.View.extend({
				 ...
				 //    	if(typeof(data.Groups)=='string'){
				    columns: [
				    	...
				    	},
				    	{
				            ...
				            "render": _.bind(function ( data, type, full, meta ) {
				            	if(typeof(full.Groups)=='string'){
				            		let Group = full.Groups.replace(/null|[\[\]"]+/g,"");
				            		if(Group==='SAIP_Admin'){
				    	...
				    	},
				    	{
				            ...
				            "render": _.bind(function ( data, type, full, meta ) {
				            	if (full.active == 1)
				            	{return "<select class='changeUserStatus' id="+full.id+" ><option value='1' selected>Active</option><option value='0'>Inactive</option></select>";}
				            	else
				            		{{return "<select class='changeUserStatus' id="+full.id+"><option value='1'>Active</option><option value='0' selected>Inactive</option></select>";}}
				            },this)
				    	},{
			            	"orderable":false,
				            "targets": -1,
				            "render": _.bind(function ( data, type, full, meta ) {
				            	// console.log(full)
				            	let phone_office = full.phone_office || null;
				            	let position = full.position || null;
				            	let email = full.email || null;
				            	let last_name = full.last_name || null;
				            	let first_name = full.first_name || null;
				            	let status = full.status || null;

				            	if(full.id !== this.user_id){
				            		// console.log(phone_office)
				            		return "<a last_name="+last_name+" first_name="+first_name+" email="+email+" phone_office="+phone_office+" position="+position+" status="+status+" userID="+full.userID+" is_pi="+full.is_pi+" user_id="+full.id+" class='fa fa-edit user_edit' style='cursor:pointer'></a>"
				            	}else{
				            		return ""
				            	}
	...
	changeUserStatus(e){
		let newUserStatus = new FormData();
	...
	changeUserPermission(e){
		...
		let group_id = e.currentTarget[e.currentTarget.selectedIndex].value

		this.selectedUser_id = $(e.currentTarget).attr('user_id');
		let selectUser = this.users.models.filter(x=>x.id==this.selectedUser_id);
		let selectUserName = selectUser[0].get('last_name')+','+selectUser[0].get('first_name');
