      ...
      success: (_.bind(function (res) {
        ...
        res.toJSON().forEach(function (sourceRow) {
          if (sourceRow.study_description != null) {
            const image_name = sourceRow.image_name
            const modality = sourceRow.modality
            // window.series_mod_time=sourceRow.series_mod_time;
            //	console.log(sourceRow);
            //	if(sourceRow.series_mod_time!=null){
            const series_mod_time = sourceRow.series_mod_time.substring(0, 10)
            //	}
            const patient = sourceRow.patient
            const patient_id = sourceRow.patient_id
            const patient_path = sourceRow.patient_path

            const serie_description = sourceRow.serie_description
            const serie_id = sourceRow.serie_id
            const serie_uid = sourceRow.serie_uid
            const serie_path = sourceRow.serie_path

            const study_id = sourceRow.study_id
            const study_path = sourceRow.study_path
            ...
            if (!this.mergedStudyDescriptionArr.some(function (row) { return row.study_description == sourceRow.study_description })) {
              ...
              const studyLevelDownload = patient_path + '/' + study_path
...
*/
          ...
          columns: [
			            { data: 'study_description' },
			            {
				            ...
				            render: _.bind(function (data, type, full, meta) {
				            	const urlBase = domain + 'api/v1/scippy'
				            	const patient_path = full.patient_path
				            	const study_path = full.study_path
				            	const study_description = full.study_description || 'Non_study_description'
                const study_id = full.study_id || 'none'
                const patient = full.patient
                const resoure = 'downloadAllStudies'
        ...
        $('.studyDownloadProgress').on('click', function (e) {
          ...
          const progressIcon = $(e.target).next('.progress')
  ...
  showChild (e) {
    ...
    const studyDes = $(e.target).closest('tr')[0].cells[0].firstElementChild.innerText

    const study = this.mergedStudyDescriptionArr.find(e => e.study_description == studyDes)

    const individulStudy = []
    var promises = []
    for (let a = 0; a < study.patient.length; a++) {
      const temp = {}
      const src = '/mnt/scippy_images/' + study.patient_path[a] + '/' + study.study_path[a] + '/' + study.serie_path[a] + '/thmb_' + study.serie_uid[a] + '.jpg'
    ...
    const mergedindividulStudy = []
    const middleStudy = individulStudy
    ...
    this.table2 = $('#showList2').DataTable({
 	...
 	width: '20%',
			            ...
			            render: function (data, type, full, meta) {
			            	...
			            	const study_path = full.study_path
            const study_id = full.study_id || 'none'
            const serie_path = full.serie_path
            const serie_uid = full.serie_uid
            const patient = full.patient
            const resoure = 'downloadStudy'
            var imgDom = ''
            ...
            const serie_path = full.serie_path
            const patient = full.patient
            const modality = full.modality
            const resoure_1 = 'downloadStudy'
            const resoure = 'downloadSeries'
    ...
    $('.seriesDownloadProgress').on('click', function (e) {
			    const progressIcon = $(e.target).prev('.progress')
      const downloadIcon = $(e.target)
      var xhr = new XMLHttpRequest()
  ...
  format (d) {
    console.log(d)
	    ...
	    for (let i = 0; i < d.patient.length; i++) {
	    	const a = '<tr>' + '<td>' + '</td>' + '<td>' + d.patient[i] + '</td>' + '<td>' + d.serie_description[i] + '</td>' + '<td>' + d.modality[i] + '</td>' + '</tr>'
	    	all += a
	    }
